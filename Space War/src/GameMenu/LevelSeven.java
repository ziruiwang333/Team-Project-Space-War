...
public class LevelSeven extends Parent {
	...
	public void LevelSeven(final Stage ThirdLevel, int power, int sc, int playerLifes,int spaceshipNumber) throws IOException {
		stage = ThirdLevel;
		score = sc;
		powerScore = power;
		...
		AnimationTimer gameLoop = new AnimationTimer() {
			...
			public void handle(long now) {
				...
				count++;
				if (count == 33) // 20 -> height of the aliens
					count = 0;
				if (number == 10) { // number of aliens to spawn on each side
					spawn = false;
				}
				shootCount++;
				if (shootCount == 100) {
				...
				removePowerups(playerBulletList);
...
//				gameOver();
		...
		scene.addEventHandler(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent t) {
				if (t.getCode() == KeyCode.F9) {
					...
					try {
						start.LevelEight(stage, powerScore, score, players.get(0).getLife(),ship);
					} catch (IOException e) {
	...
	private void enemiesMove(List<? extends Sprite> spriteList) {
		Iterator<? extends Sprite> iter = spriteList.iterator();
		counter++;
		int c = rnd.nextInt(enemies.size() + 1);
		double turnRate = 0.6;
		double speed = 4;
		...
		while (iter.hasNext()) {
			...
			Sprite sprite = iter.next();
			if (c == spriteList.indexOf(sprite) && counter == 300) {
				counter = 0;
	...
	private void Stop(List<? extends Sprite> SpriteList) {
		Iterator<? extends Sprite> iter = SpriteList.iterator();
		while (iter.hasNext()) {
			Sprite sprite = iter.next();
			if (sprite.getX() >= 1050) {
				enemies.forEach(sprite1 -> sprite1.setDx(0));
				perm = false;
			}
		}
	}
	...
	public void spawnEnemies(boolean random) {
		...
		var alien1 = new Alien(playfieldLayer, image, 0, y1, 0, 3, 0, 0, 20.0, 1, 0, 0);
		var alien2 = new Alien(playfieldLayer, image, 0, y2, 0, 3, 0, 0, 20.0, 1, 0, 0);
		var alien3 = new Alien(playfieldLayer, image, 0, y3, 0, 3, 0, 0, 20.0, 1, 0, 0);
		var alien4 = new Alien(playfieldLayer, image, 0, y4, 0, 3, 0, 0, 20.0, 1, 0, 0);
		...
		enemies.add(alien4);
		/*
		 * enemiesLeft.add(alienleft); enemiesRight.add(alienright); var alienleft1 =
		 * new Alien(temporar,image,xleft, yleft, 0, 0, 2, 0, 10.0,1,0,0); var
		 * alienright1 = new Alien(temporar,image,xright, yright, 0, 0, 2, 0,
		 * 10.0,1,0,0);
		 */
