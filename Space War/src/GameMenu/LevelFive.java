...
public class LevelFive extends Parent {
	...
	Text playerLifeText = new Text();
	MultiPlayerLife life1;
	MultiPlayerLife life2;
	int playerLifes;

	public void LevelFive(final Stage ThirdLevel, int power, int sc, int playerLifes,int spaceshipNumber) throws IOException {
		score = sc;
		stage = ThirdLevel;
		powerScore = power;
		...
		AnimationTimer gameLoop = new AnimationTimer() {
			...
			public void handle(long now) {
				...
				count++;
				if (count == 100) // number of frames between each spawning
					count = 0;
...
//				gameOver();
		...
		scene.addEventHandler(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent t) {
				if (t.getCode() == KeyCode.F9) {
					...
					try {
						start.LevelSix(stage, powerScore, score, players.get(0).getLife(),ship);
					} catch (IOException e) {
	...
	public void spawnEnemies(boolean random) {
		...
		var alien = new Alien(playfieldLayer, image, x, y, 0, 0, speed, 0, 16.0, 1, 0, 0);
		enemies.add(alien);
		tempList.add(alien);
	...
	public void SpriteShoot(List<? extends Sprite> spriteList) {
		...
		while (i.hasNext()) {
			...
			if (temp == mark) {
				...
				enemyBullet = new Shot(playfieldLayer, image, x, y, 0, sprite.getDy() + 1, 0, 0);
