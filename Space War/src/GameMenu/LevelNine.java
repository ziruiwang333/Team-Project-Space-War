...
public class LevelNine extends Parent {
	...
	public void LevelNine(final Stage ThirdLevel, int power, int sc, int playerLifes,int spaceshipNumber) throws IOException {
		score = sc;
		...
		AnimationTimer gameLoop = new AnimationTimer() {
			...
			public void handle(long now) {
				...
				if (count == 0 && spawn) {
					...
					moveInfinity(button, spawn);
					spawnEnemies(spawn);
					number++;

				}
				count++;
				if (count == 20) // 20 -> height of the aliens
					count = 0;
...
//				gameOver();
		...
		scene.addEventHandler(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent t) {
				if (t.getCode() == KeyCode.F9) {
					...
					try {
						start.BossLevel(stage, powerScore, score, players.get(0).getLife(),ship);
					} catch (IOException e) {
	...
	private void moveInfinity(Button button, boolean spawn) {
