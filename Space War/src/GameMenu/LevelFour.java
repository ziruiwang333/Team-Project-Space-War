...
public class LevelFour extends Parent {
	...
	private List<Sprite> tempList = new ArrayList<>();
	...
	public void LevelFour(final Stage FourthLevel, int power, int sc, int playerLifes, int spaceshipNumber) throws IOException {
		stage = FourthLevel;
		score = sc;
		...
		AnimationTimer gameLoop = new AnimationTimer() {
			...
			public void handle(long now) {
				...
				if (count == 0 && spawn) {
					...
					playfieldLayer.getChildren().add(button);
					button.setVisible(true);
					moveCircle(button, spawn);
				...
				count++;
				if (count == 25) // 20 -> height of the aliens
					count = 0;
				if (number == 19) { // number of aliens to spawn
					spawn = false;
				}
				shootCount++;
				if (shootCount == 100) {
...
//				gameOver();
		...
		scene.addEventHandler(KeyEvent.KEY_PRESSED, new EventHandler<KeyEvent>() {
			...
			public void handle(KeyEvent t) {
				if (t.getCode() == KeyCode.F9) {
					...
					try {
						start.LevelFive(stage, powerScore, score, players.get(0).getLife(),ship);
					} catch (IOException e) {
	...
	private void getCoords(List<Button> buttons) {
		...
		while (iter.hasNext()) {
			...
			if (boundsInScene.getCenterX() != 620 && boundsInScene.getCenterY() != 300) {
				Sprite sprite = enemies.get(index);
	...
	private void moveCircle(Button button, boolean spawn) {
	...
	public void spawnEnemies(boolean random) {
		...
		var alien = new Alien(playfieldLayer, enemyImage, x, y, 90, 0, 1, 0, 20, 1, 0, 0);
		enemies.add(alien);
